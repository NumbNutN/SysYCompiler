...
void print_operand(AssembleOperand op, size_t opernadIdx) {
  ...
  */
  ...
  case REGISTER_DIRECT:
    ...
    else if (op.oprendVal >= FIRST_VFP_REGISTER &&
             op.oprendVal <= LAST_VFP_REGISTER)
  ...
  case REGISTER_INDIRECT_POST_INCREMENTING:
    ...
    if (op.oprendVal >= FIRST_ARM_REGISTER && op.oprendVal <= LAST_ARM_REGISTER)
      printf("[R%d], #%d", (int)op.oprendVal, op.addtion);
             ...
             op.oprendVal <= LAST_VFP_REGISTER)
      printf("[S%d], #%d", (int)op.oprendVal,
             op.addtion - FLOATING_POINT_REG_BASE);
    else
  ...
  case REGISTER_INDIRECT_WITH_OFFSET: {
    if (op.offsetType == OFFSET_IMMED || op.offsetType == NONE_OFFSET) {
      if (op.oprendVal >= FIRST_ARM_REGISTER &&
          op.oprendVal <= LAST_ARM_REGISTER)
        printf("[R%d, #%d]", (int)op.oprendVal, op.addtion);
      else if (op.oprendVal >= FIRST_VFP_REGISTER &&
               op.oprendVal <= LAST_VFP_REGISTER)
        printf("[S%d, #%d]", (int)op.oprendVal,
      ...
      if (op.oprendVal >= FIRST_ARM_REGISTER &&
        ...
        printf("[R%d, R%d]", (int)op.oprendVal, op.addtion);
               ...
               op.oprendVal <= LAST_VFP_REGISTER)
        printf("[S%d, R%d]", (int)op.oprendVal,
               op.addtion - FLOATING_POINT_REG_BASE);
      else
